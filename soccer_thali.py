...
with open ('2.tsv','w') as f1:
	...
	wanted_keys = ['Task_Name','Task_Type','External_id','Description','URL']
	final_dict=OrderedDict([(a,dd2[a]) for a in wanted_keys if a in dd2])
	#print final_dict.items()
	l =[]
	l=final_dict['External_id']
	u = final_dict['URL']
	print 'External Ids LIST : ',l
	#print final_dict['External_id']
	for rows in final_dict['External_id']:
		print 'External ids in final_dict : ', rows
		for m in l:
			if m in rows:
				print 'true', m
				for ul in u:
