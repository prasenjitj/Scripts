    ...
    for i in range(len(response)):
        try:
            ...
            if title_match:
                ...
                print title_match
                title_match = title_match[0].lower().strip().decode('utf-8')
                print title_match
            ...
            if director_match is not None:
                ...
                print director_match
                director_match =  director_match[0].lower().strip().decode('utf-8')
                print director_match
            print  'title is :: %s and director is :: %s' %(title,director)
            print  'title_match is :: %s and director_match is :: %s' %(title_match,director_match)
            if title in title_match and director_match is not None and director in director_match:
                imdb_url = response[i].get('pagemap').get('metatags')[0].get('og:url')
                ...
                imdb_url = response[i]['pagemap']['metatags'][0].get('og:url')
                writer.writerow({'Entity': Entity, 'Director': Director, 'Alias': Alias, 'IMDB URL': imdb_url})
                print imdb_url
                flag = 1
                break
            elif title in title_match:
                imdb_url = response[i].get('pagemap').get('metatags')[0].get('og:url')
                writer.writerow({'Entity': Entity, 'Director': Director, 'Alias': Alias, 'IMDB URL': imdb_url})
                print imdb_url
                flag = 1
...
api_keys = ['AIzaSyD6cpIbG5BZz5fGo0NeHjPPgSu1PoWnS48',
        ...
        for i in api_keys:
          for j in range(1, 81):
            ...
            for row in reader:
                ...
                director = row['Director'].lower().strip().encode('utf-8')
